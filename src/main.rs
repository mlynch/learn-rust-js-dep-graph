...
use swc_ecma_ast::{Program, Module, Stmt, Decl, ModuleDecl, ExportDecl, ImportDecl};
  ...
  fn _push(&mut self, file_name: String, dep: String) {
    if self.map.contains_key(&file_name) {
      ...
      if let Some(v) = m.get_mut(&file_name) {
        println!("Pushing dep {} {}", file_name, dep);
        v.push(dep);
      }
    } else {
      let  v = vec![dep];
...
fn parse(file_name: &str, parser: &mut Parser<Lexer<StringInput>>, handler: Handler, graph: &mut Graph) {
